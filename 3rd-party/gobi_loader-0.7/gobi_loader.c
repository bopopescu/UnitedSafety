...
char magic4_verizon[] = {0x25, 0x06, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00,
                 0x00, 0x00, 0x04, 0x00, 0x00, 0x25, 0x7d, 0x5e};
...
static void h_set_int(void *p_des, int p_i)
{
...
static void h_set_short(void *p_des, short p_s)
{
	unsigned char *des = (unsigned char *)p_des;
	des[0] = p_s & 0xff;
	des[1] = p_s >> 8 & 0xff;
...
int main(int argc, char **argv) {	
	...
	h_set_int(magic2 + 2, file_data.st_size - 8);
	h_set_int(magic3 + 7, file_data.st_size - 8);
	h_set_short(magic1 + sizeof(magic1)-2, ~crc_ccitt(0xffff, magic1, sizeof(magic1)-2));
	h_set_short(magic2 + sizeof(magic2)-2, ~crc_ccitt(0xffff, magic2, sizeof(magic2)-2));
	h_set_short(magic3 + sizeof(magic3)-2, ~crc_ccitt(0xffff, magic3, sizeof(magic3)-2));
	...
	fstat(fwfd, &file_data);
	{
		const int verizon_work_around = (3141676 == file_data.st_size);
		struct Magic {
		int m_size;
		char *m_data;
		};

		struct Magic verizon;
		struct Magic other;
		struct Magic *magic = &other;

		other.m_size = sizeof(magic4);
		other.m_data = magic4;
		verizon.m_size = sizeof(magic4_verizon);
		verizon.m_data = magic4_verizon;

		if(verizon_work_around) magic = &verizon;

		h_set_int(magic->m_data + 2, file_data.st_size);
		h_set_int(magic5 + 7, file_data.st_size);
		if(!verizon_work_around) h_set_short(magic->m_data + magic->m_size-2, ~crc_ccitt(0xffff, magic4, magic->m_size-2));
		h_set_short(magic5 + sizeof(magic5)-2, ~crc_ccitt(0xffff, magic5, sizeof(magic5)-2));

		write (serialfd, "\x7e", 1);
		write (serialfd, magic->m_data, magic->m_size);
		write (serialfd, "\x7e", 1);
	}
	...
	if (gobi2000) {
		...
		fstat(fwfd, &file_data);
		h_set_int(magic6 + 2, file_data.st_size);
		h_set_int(magic7 + 7, file_data.st_size);
		h_set_short(magic6 + sizeof(magic6)-2, ~crc_ccitt(0xffff, magic6, sizeof(magic6)-2));
		h_set_short(magic7 + sizeof(magic7)-2, ~crc_ccitt(0xffff, magic7, sizeof(magic7)-2));
	...
	h_set_short(magic8 + sizeof(magic8)-2, ~crc_ccitt(0xffff, magic8, sizeof(magic8)-2));
