...
int g_hid_voltage_channel = 3;
module_param(g_hid_voltage_channel, int, 0);

int get_hid_voltage(void)
{
	uint32_t  ch8Value;
	const uint32_t channel = g_hid_voltage_channel;
...
static int __init init(void)
{
	...
	int ret;
	ret = register_chrdev_region( dev, NMINOR, "hid-voltage");
	if( ret) {
	...
	__raw_writel(BF_LRADC_CTRL2_DIVIDE_BY_TWO(1 << g_hid_voltage_channel),
			REGS_LRADC_BASE + HW_LRADC_CTRL2_SET);

	printk(KERN_NOTICE "TRULink Hardware ID Voltage driver loaded");
...
static void __exit fini(void)
{
	...
	unregister_chrdev_region( MKDEV( UMAJOR, 1), NMINOR);

	printk(KERN_NOTICE "TRULink Hardware ID Voltage driver unloaded");
}
...
MODULE_LICENSE( "GPL");
MODULE_DESCRIPTION( "Read Hardware ID Voltage for TRULink");
MODULE_AUTHOR(	"Lee Wei <lwei@gps1.com>");
