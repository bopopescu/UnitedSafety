...
extern int g_trulink_2500_magic_usb_irq_93_work_around;
#endif
...
static irqreturn_t ehci_fsl_pre_irq(int irq, void *dev)
{
	...
	if (!test_bit(HCD_FLAG_HW_ACCESSIBLE, &hcd->flags)) {
		...
		if (pdata->irq_delay || ((pdata->wakeup_event == WAKEUP_EVENT_VBUS) || (pdata->wakeup_event == WAKEUP_EVENT_INVALID)))
		{
			...
			#ifdef CONFIG_TRULINK_2500_MAGIC_USB_IRQ_93_WORK_AROUND
			// AWARE360 FIXME: This is magic code. IRQ 93 will be disabled by the Kernel after 100,000 (do-nothing)
			//	signals if this code is not here on TRULink 2500 devices. The TRULink 3000 has no issues with IRQ
			//	93. Explain this difference.
			if((93 == irq) && g_trulink_2500_magic_usb_irq_93_work_around)
			{
