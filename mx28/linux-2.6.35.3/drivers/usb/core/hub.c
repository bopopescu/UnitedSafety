	...
	struct usb_endpoint_descriptor *endpoint)
{
	...
	if (hdev == hdev->bus->root_hub) {
		...
		/* REDSTONE FIXME: [RedStone Bug 34] Don't use this work-around, fix the problem. */
		#ifdef CONFIG_REDSTONE_BUG_34_WORK_AROUND
		hub->mA_per_port = 500;
		printk(KERN_NOTICE "\x1b[1;44;37m%s,%d:%s: REDSTONE BUG 34 WORK-AROUND APPLIED for \"%s\", devnum=%d, (%s,%s,%s)\x1b[0m\n", __FILE__, __LINE__, __FUNCTION__,
			hub->hdev->devpath,
			hub->hdev->devnum,
			hub->hdev->product,
			hub->hdev->manufacturer,
			hub->hdev->serial);
		#else
		...
		#endif /* CONFIG_REDSTONE_BUG_34_WORK_AROUND */
