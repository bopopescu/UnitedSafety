...
static int __devinit mxs_wdt_probe(struct platform_device *pdev)
{
	...
	/*
	  REDSTONE XXX: Bug #67 in itracker.atsplace.com
		Disabling watchdog = no watchdog at all.

		Disabling a watchdog is insane (why even code the watchdog at all)?
		The disabled watchdog bug was first discovered when the Kernel booted fine,
		but then "bricked" the device when Network File System (NFS) failed (Kernel
		panicked cause it could not find a proper root filesystem). A watchdog reset
		would have eventually caused the device to use its NAND root filesystem,
		thus saving the device.
	*/
	if(g_redstone_wdt_disable)
	{
		printk(KERN_EMERG "%s,%d: RedStone i.MX28 WatchDog is disabled due to request from Kernel cmdline.\n"
			"NOTE: Simply write to /dev/watchdog to start it again.\n",
			__FILE__, __LINE__);
