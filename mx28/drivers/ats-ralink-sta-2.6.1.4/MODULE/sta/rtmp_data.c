	...
	IN RX_BLK *pRxBlk)
{
	...
	if ((pHeader->FC.FrDs == 1) && (pHeader->FC.ToDs == 1)) {
			...
			&& (pHeader->FC.SubType & 0x08))
		{
			...
			if ((*pData >> 4) & 0x01)
			{
				...
				/* Force driver to fall into sleep mode when rcv EOSP frame */
					...
					{
#ifdef DOT11Z_TDLS_SUPPORT
						if ((pAd->StaCfg.TdlsInfo.TdlsForcePowerSaveWithAP) && (RtmpPktPmBitCheck(pAd) == FALSE))
						{
							DBGPRINT(RT_DEBUG_TRACE, ("TDLS STA awake can not sleep when do channel switch !!\n"));
						}
						else
#endif /* DOT11Z_TDLS_SUPPORT */
						{
		...
		if (IS_ENTRY_TDLS(pEntry) &&
			(pAd->StaCfg.TdlsInfo.bChannelSwitchWaitSuccess))
		{
			PRT_802_11_TDLS pTDLS = NULL;
			BOOLEAN TimerCancelled;

			pAd->StaCfg.TdlsInfo.bChannelSwitchWaitSuccess = FALSE;
			pTDLS = &pAd->StaCfg.TdlsInfo.TDLSEntry[pEntry->MatchTdlsEntryIdx];
