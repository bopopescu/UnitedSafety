...
extern wait_queue_head_t g_wifi_q;
extern atomic_t* g_wifi_irq;
static ULONG prevTransmittedByteCount = 0;
...
/*				printk("data bulk out bLasAlignmentsectiontRound \n");*/
		...
		if( pAd->RalinkCounters.TransmittedByteCount - prevTransmittedByteCount > 1000 )
		{
			atomic_set(g_wifi_irq, 1);
			wake_up_interruptible(&g_wifi_q);
			prevTransmittedByteCount = pAd->RalinkCounters.TransmittedByteCount;
		}
