	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	p2pindex = P2pGroupTabSearch(pAd, DevAddr);
	{
		...
		if (p2pindex!=P2P_NOT_FOUND)
		{
			...
			pP2pEntry = &pAd->P2pTable.Client[p2pindex];
			/* 
				Also update the bssid, because ioctl query form OID_802_11_WFD_PEER_RTSP_PORT will not search p2p table by device address
				So userspace will get the wrong RTSP port (WFD_RTSP_DEFAULT_PORT)
			*/
			RTMPMoveMemory(pP2pEntry->bssid, BssidAddr, MAC_ADDR_LEN);
			if (WfdSubelementLen)
			{
				WfdParseSubElmt(pAd, 
								&pP2pEntry->WfdEntryInfo,
