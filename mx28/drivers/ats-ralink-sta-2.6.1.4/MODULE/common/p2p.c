		...
		if (pP2PCtrl->WscMode == WSC_PIN_MODE)
		{
			if (pP2PCtrl->ConfigMethod == WSC_CONFMET_KEYPAD)
			{
				...
				/*
					...
					If use sigma tool (p2p & Miracast certified), UI/wpa_supplicant will be disabled,
					so the ConfirmByUI will not be toggled.
				*/
			...
			else if (pP2PCtrl->ConfigMethod == WSC_CONFMET_DISPLAY)
			{
				...
				OS_WAIT(700);
				Set_AP_WscGetConf_Proc(pAd, "1");
				/* Set bConfirmByUI to default (=FALSE), otherwise next connection request will not be passed to UI when previous connection type is DISPLAY. */
				pP2PCtrl->bConfirmByUI = FALSE;
			}
		}
