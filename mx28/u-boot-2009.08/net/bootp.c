...
#define TIMEOUT		1000UL	/* Milliseconds before trying BOOTP again */
...
DhcpHandler(uchar * pkt, unsigned dest, unsigned src, unsigned len)
{
	...
	case REQUESTING:
		...
		if ( DhcpMessageType((u8 *)bp->bp_vend) == DHCP_ACK ) {
			...
			dhcp_state = BOUND;
			printf ("\x1b[1;44;37mDHCP client bound to address %pI4\x1b[0m\n", &NetOurIP);
			...
			if(strncmp("192.168.28.", &NetOurIP, 11)) {
				printf ("\x1b[1;44;37mOn Test-Fixture local network, downloading firmware\x1b[0m\n");
				TftpStart();
			}
			return;
		}
