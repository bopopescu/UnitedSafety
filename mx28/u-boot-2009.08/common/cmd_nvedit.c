...
#if (defined(CONFIG_ENV_IS_IN_NAND) && defined(CONFIG_SICONIX_UPGRADE_SYS))
int do_savedef (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	printf("Saving Environment to factory default storage...\n");
	return (saveenv_def() ? 1 : 0);
}

int do_loaddef (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	printf("Loading Environment from factory default storage...");
	if (loadenv_def(1))
	{
		printf("FAILED!\n");
		return 1;
...
#endif /* #if CONFIG_ENV_IS_IN_NAND && CONFIG_SICONIX_UPGRADE_SYS */
...
#if (defined(CONFIG_ENV_IS_IN_NAND) && defined(CONFIG_SICONIX_UPGRADE_SYS))
U_BOOT_CMD(
	savedef, 1, 0,	do_savedef,
	"savedef - save environment variables to default persistent storage\n",
	NULL
);

U_BOOT_CMD(
	loaddef, 1, 0,	do_loaddef,
	"loaddef - load environment variables from default persistent storage\n",
	NULL
);
#endif /* #if CONFIG_ENV_IS_IN_NAND && CONFIG_SICONIX_UPGRADE_SYS */
