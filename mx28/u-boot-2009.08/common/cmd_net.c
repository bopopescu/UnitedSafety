...
int g_watch_for_trulink_test_fixture_signal = 0;
...
#endif
int do_fixture (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	printf("Checking for TRULink Test Fixture......\n");
	g_watch_for_trulink_test_fixture_signal=1;

	if (NetLoop(TRULINK_TEST_FIXTURE) < 0) {
		g_watch_for_trulink_test_fixture_signal=0;
		printf("Connecting to TRULink Test Fixture failed\n");
		return 1;
	}
...
U_BOOT_CMD(
	fixture,	2,	1,	do_fixture,
	"Connect to TRULink Test Fixture",
	"arg"
);
