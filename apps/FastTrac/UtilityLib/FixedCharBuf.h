/*$============================================================================
  Name:   FixedCharBuf - implement a fixed size (2048) circular character buffer
==============================================================================*/
#pragma once
...
private:
  ...
  char  m_Elements[2048];   // allocated array of m_Elements of type T

public:

  //---------------------------------------------------------------------------
  FixedCharBuf(void)                   // Constructor
  {
    m_Head = 0;
    m_Tail = 0;
    m_nElements = 0;
  ...
  //---------------------------------------------------------------------------
  // Delete num T's from the FixedCharBuffer - starting at the specified char
  ...
  //---------------------------------------------------------------------------
  // Remove num chars from the FixedCharBuffer - starting at the m_Tail
  ...
  //---------------------------------------------------------------------------
  // Remove 1 T from the FixedCharBuffer - starting at the m_Tail
  ...
  //---------------------------------------------------------------------------
  // Remove num T's from the FixedCharBuffer - starting at the m_Tail - don't fill an output buffer
  bool Toss(long num)
  {
    if (m_nSize == 0 || num < 1L)
      return false;
...
;
  ...
  short GetLine(char *strLine, short maxChars)
  {
    ...
    if (cr + 1 > maxChars)
    {
//      printf("FixedCharBuf::GetLine cr found at %d\n",cr);
      Delete(0, cr);
      return -2;
    }
  ...
  void DumpHex()
  {
    printf("FixedCharBuf Dump -->>  Num Elements = %ld  tail %ld head %ld\r\n", m_nElements, m_Tail, m_Head);
    long scan_m_Tail = (m_Tail) % m_nSize;
