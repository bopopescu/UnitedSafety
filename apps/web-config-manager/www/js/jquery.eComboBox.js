...
(function( $ ){
  ...
  var methods = {
     init : function( options ) {
		...
		return this.each(function() { 
			...
			if (settings.allowNewElements){
				selectElement.prepend('<option>Add to list</option>');
			}
			...
			selectElement.keydown( function(e){
				...
				if( selected=="Add to list" || settings['editableElements'] ) {
	 ...
	 enableAddingNewElements: function(){
		var fChild = $(this).children().first();
		if(fChild.text() != "Add to list"){
			$(this).prepend('<option>Add to list</option>');
		}
	 },
