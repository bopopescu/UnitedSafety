...
InetDB::InetDB(MyData& pData, const ats::String& p_packetizerdb_name, const ats::String& p_packetizerdb_path) :
...
bool InetDB::copy(const ats::String& p_table, ats::StringMap &p_sm)
{
...
//----------------------------------------------------------------------
//	Copy the record for mid from the message table to the inet table.
//	Only copies if the event_type is 254 (inet_msg)
//
bool InetDB::CopyRecordFromMessageTable(int p_mid)
{
	ats::StringMap sm;
	db_monitor::DBMonitorContext db;
...
//----------------------------------------------------------------------
void InetDB::dbselectbacklog(std::vector<int>& p_mid, int p_limit)
{
	db_monitor::DBMonitorContext db;
	{
		ats::String buf;
		ats_sprintf(&buf, "SELECT mid FROM message_table where event_type = '254' ORDER BY mid ASC LIMIT %d", p_limit);
...
//-------------------------------------------------------------------------------------------------
//
// loads the message_types table into a map of [mid,calamp_id] pairs
//
bool InetDB::LoadMessageTypes(std::map<int,int>& p_mt)
{
	...
	//ats_logf(ATSLOG_INFO, "%s, %d: Database result returned: Table size:%lu", __FILE__, __LINE__, db.Table().size());
...
//-------------------------------------------------------------------------------------------------
int InetDB::SelectSinglePriorityOneMessage()
{
	ats::String buf;
...
//-------------------------------------------------------------------------------------------------
// remove records over 12 hours old
void InetDB::CleanupDB()
{
	db_monitor::DBMonitorContext db;
	dbquery(db, m_packetizerdb_name, "DELETE FROM message_table WHERE event_time < datetime('now', '-43200 seconds'); ");
//	Test();
}
...
/*
	...
	ats_logf(ATSLOG_INFO, "Result of  count - table size %d.  Records:%d", (int)db.Table().size(), val);
*/
}
