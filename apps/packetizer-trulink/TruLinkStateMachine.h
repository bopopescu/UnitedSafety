...
class TruLinkStateMachine : public StateMachine
{
public:
	TruLinkStateMachine(MyData&);
	void start();
	bool BuildTLPEventBuffer(int p_mid, struct send_umessage& p_msg);
	void get_message_backlog(int p_limit);

private:

	void (TruLinkStateMachine::*m_state_fn)();
	void state_0();
	void state_1();
	void state_2();
	void state_3();
	...
	std::vector<struct send_umessage> m_OutputMsg;
	struct message_info m_RecordData;
	...
	std::vector<unsigned char> m_state15_iridium_data;
