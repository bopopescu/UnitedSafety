...
#define STATE_MACHINE_CLASS TruLinkCopy
#define SET_NEXT_STATE(P_STATE) m_state_fn = &STATE_MACHINE_CLASS::state_ ## P_STATE; m_state = P_STATE

COMMON_STATE_MACHINE_START_FUNCTION_DEFINITION(TruLinkCopy)

TruLinkCopy::TruLinkCopy(MyData& mdata): StateMachine(mdata),m_data(&mdata)
{
...
TruLinkCopy::~TruLinkCopy()
{
	delete m_dbreader;
}

void TruLinkCopy::state_0()
{
	MyData& md = dynamic_cast<MyData&>(my_data());
