...
#define CAN_CODE_FORD_SEATBELT (0x48)
#define FORD_SEATBELT_MASK 0x20
int g_dbg = 0;
...
static void* can_polling(void* p)
{
	...
	for(;;)
	{
		...
		const int FORD_CAN_POLL_ID = md.get_int("FORD_CAN_POLL_ID");
		CANSocket* s = md.m_s;
		for(;;)
		{
			{
				...
				data[3] = 0x48;
				g_can_write_line = __LINE__;
				CAN_WriteResult res = CAN_write(s, FORD_CAN_POLL_ID, data, 8);
...
void get_processed_can_data(ats::String& p_result, struct can_frame p_msg)
{
	...
	if((p_msg.can_id & 0xFFFFFF00) != 0x73F)
	{
		return;
	}
	...
	if(!((p_msg.data[1] == 0x62) && (p_msg.data[0] < 7) && (p_msg.data[0] >=3)))
	{
		return;
	}

	const int val = (p_msg.data[4]&FORD_SEATBELT_MASK)? 1:0;

	switch(p_msg.data[3])
	{
	case CAN_CODE_FORD_SEATBELT:
...
static void* can_message_handler(void* p)
{
	...
	for(;;)
	{
		...
		if(!((msg.data[1] == 0x62) && (msg.data[2] == 0x59) && (msg.data[0] < 7) && (msg.data[0] >=3)))
		{
			continue;
		}

		g_can_message_handler_line = __LINE__;
		int val = (msg.data[4] & FORD_SEATBELT_MASK)? 1 : 0;
		switch(msg.data[3])
		{
		case CAN_CODE_FORD_SEATBELT:
			g_can_message_handler_line = __LINE__;
	...
	for(;;)
	{
		...
		if(g_has_work)
		{
			...
			if(!work_was_set)
			{
				g_has_work_wait_line = __LINE__;
				work_was_set = true;
				ats::system("echo \"set_work key=can-ford-seatbelt-monitor\"|telnet localhost 41009");
		...
		else
		{
			...
			if((seconds_without_data >= two_minutes) && work_was_set)
			{
				g_has_work_wait_line = __LINE__;
				work_was_set = false;
				ats::system("echo \"unset_work key=can-ford-seatbelt-monitor\"|telnet localhost 41009");
...
int start_application(MyData& p_md, int argc, char* argv[])
{
	...
	md.set("FORD_CAN_POLL_ID", "0x737");
	md.set("FORD_CAN_RX_ID","0x73F");
...
{
	g_log.open_testdata("can-ford-seatbelt-monitor");
