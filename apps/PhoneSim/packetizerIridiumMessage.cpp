...
void PacketizerIridiumMessage::packetize(std::vector<char> &data)
{
	...
	ats_logf(ATSLOG_INFO, "%s,%d: packetize msgCode=%d", __FILE__,__LINE__, m_MsgCode);

	switch(m_MsgCode)
	{
	case TRAK_HEARTBEAT_MSG:
	{
		ats_logf(ATSLOG_INFO, "%s,%d: TRAK_HEARTBEAT_MSG", __FILE__, __LINE__);
	...
	case TRAK_PING_MSG:
	{
		...
		{
			ats_logf(ATSLOG_INFO, "%s,%d: TRAK_SCHEDULED_MSG/PING _ESN", __FILE__, __LINE__);
		...
		{
			ats_logf(ATSLOG_INFO, "%s,%d: TRAK_SCHEDULED_MSG/PING", __FILE__, __LINE__);
	...
	case TRAK_CALAMP_USER_MSG:
	{
		...
		if(!m_esn.empty() && (m_esn != m_cell_imei))
		{
			ats_logf(ATSLOG_INFO, "%s,%d: TRAK_CALAMP_USER_MSG_ESN with Iridium IMEI", __FILE__, __LINE__);
			((FTData_UserData_ESN *)myEvent)->SetData(m_nmea, m_MsgUserData, m_esn, m_esn_type);
		}
		else
		{
			ats_logf(ATSLOG_INFO, "%s,%d: TRAK_CALAMP_USER_MSG_ESN with cell  IMEI", __FILE__, __LINE__);
			((FTData_UserData_ESN *)myEvent)->SetData(m_nmea, m_MsgUserData, m_cell_imei, CalAmpsMessage::OptionsHeader::MOBILE_ID_IMEI);
		}
		break;
	}
	default:
		ats_logf(ATSLOG_ERROR, "%s,%d: Failed to packetize: event = %d", __FILE__, __LINE__, m_MsgCode);
		return;
	}
