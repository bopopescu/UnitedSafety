...
bool PacketizerCellSender::m_bAlarmMonitor;
...
PacketizerCellSender::PacketizerCellSender(MyData& pData): PacketizerSender(pData)
{
	...
	m_bAlarmMonitor = (bool)db.GetInt("feature", "alarm-monitor", 0);
	m_bIsPrimary = false;
...
bool PacketizerCellSender::packetizeMessageInfo(message_info* p_mi, std::vector<char>& p_data, std::map<int,int>& p_msgTypes)
{
	...
	else
	{
		...
		ats_logf(ATSLOG_DEBUG, "%s,%d: Read Event Message %d for seq num %d", __FILE__, __LINE__, p_msgTypes[code], p_mi->seq);
	}
...
ssize_t PacketizerCellSender::sendUdpData(ServerData& p_sd, const char *p_data, uint p_data_length)
{
	...
	if( ret != 0)
	{
		ats_logf(ATSLOG_DEBUG, "%s,%d:getaddrinfo error:%d - %s",__FILE__,__LINE__,ret, gai_strerror(ret));
		if(ret == EAI_SYSTEM)
		{
			ats_logf(ATSLOG_DEBUG, "%s,%d:EAI_SYSTEM Error %d -%s", __FILE__,__LINE__, errno, strerror(errno));
		}
		return -2;
	}
...
bool PacketizerCellSender::sendSingleMessage(message_info* p_mi, std::map<int,int>& p_msgTypes)
{
	...
	if(nsend < (ssize_t)(write_data.size()))
	{
		if (nsend >= 0)
		{
			ats_logf(ATSLOG_ERROR, "%s,%d: Error sending message. Sent %d of %d bytes", __FILE__, __LINE__, nsend, write_data.size());
		}
		else if(nsend == -1)
		{
			ats_logf(ATSLOG_ERROR, "%s,%d: Error sending message. Error code: %d - %s", __FILE__, __LINE__, errno, strerror(errno));
		}
		return false;
	}
	lock();
...
int PacketizerCellSender::waitforack()
{
	...
	if(err == -1)
	{
		if (errno == ETIMEDOUT)
		{
			ats_logf(ATSLOG_DEBUG, "%s,%d : " RED_ON " Timeout by sem_timedwait() - waited %d seconds." RESET_COLOR, __FILE__, __LINE__, m_timeout);
			return 0;
		}
		else
			ats_logf(ATSLOG_DEBUG, "%s,%d : Error returned by sem_timedwait() - %d:%s", __FILE__, __LINE__, errno, strerror(errno));
...
void PacketizerCellSender::processMessage(QByteArray &data,ServerData &p_sd)
{
	...
	case CalAmpsMessage::MessageHeader::MessageType::USER_DATA:
		{
			...
			ats_logf(ATSLOG_INFO, "%s,%d: Calamp User Message Detected. Calculated size=%d. Actual Size=%d", __FILE__, __LINE__, msg_length, data.size());
	...
	case CalAmpsMessage::MessageHeader::MessageType::ACK_NACK:
		{
			...
			if(seq_number == read_seq)
			{
				...
				ats_logf(ATSLOG_DEBUG, "%s,%d: Calamp User Ack is invalid seq_number=%d. read_seq=%d", __FILE__, __LINE__, seq_number, read_seq);
	...
	if( id != m_defaultMobileId)
	{
		...
		ats_logf(ATSLOG_INFO, "%s,%d: Mobile ID of message %s", __FILE__, __LINE__, id.c_str());
		db_monitor::ConfigDB db;
		ats::String socket = db.GetValue("devices", id);

		if(!socket.empty())
		{
			ats_logf(ATSLOG_DEBUG, "%s,%d: Sending to socket: %s, data:##%s##", __FILE__, __LINE__, socket.c_str(), data.toHex().data());
	...
	else if (m_bAlarmMonitor)
	{
		ats_logf(ATSLOG_DEBUG, "%s,%d: Sending to alarm-monitor, data:##%s##", __FILE__, __LINE__,  data.toHex().data());
		send_redstone_ud_msg("alarm-monitor", 0, "calamp data=%s sender=packetizer-cams\r", data.toHex().data());
	}
...
void PacketizerCellSender::sendAck(ServerData& p_sd, QByteArray& msg)
{
	...
	ats_logf(ATSLOG_DEBUG, "%s,%d: Sending Ack", __FILE__, __LINE__);
...
void* PacketizerCellSender::processUdpData(void* p)
{
	...
	for(;;)
	{
		...
		ats_logf(ATSLOG_INFO, " %s,%d: --[UDP data from %s]--", __FILE__, __LINE__,inet_ntoa(src_addr.sin_addr));
		std::stringstream ss;
		ss << "RECV[" << nread << "]:";
		...
		ats_logf(ATSLOG_INFO, "%s, %d: %s", __FILE__, __LINE__, ss.str().c_str());
...
{
	ats_logf(ATSLOG_INFO, "Connected to %s", m_host.c_str());
}
...
{
	ats_logf(ATSLOG_INFO, "%s,%d: Disconnected from %s", __FILE__, __LINE__, m_host.c_str());
}
...
void PacketizerCellSender::BroadcastToSecondary(QByteArray &data)
{
	try
	{
		...
		if (!error)
		{
			...
			socket.close(error);
			ats_logf(ATSLOG_INFO, "%s,%d: Broadcast data on ip %s:%d -> sent %d bytes", __FILE__, __LINE__, strIP.c_str(), m_BroadcastPort, data.size());
		}
	}
