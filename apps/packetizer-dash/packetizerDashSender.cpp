...
PacketizerDashSender::PacketizerDashSender(MyData& p_data) : PacketizerSender(p_data)
{
	db_monitor::ConfigDB db;
	m_host = db.GetValue("packetizer-dash", "host", "redstone.atsplace.com");
	m_port = db.GetInt("packetizer-dash", "port", "40556");
...
bool PacketizerDashSender::connect()
{
	m_connected = (!connect_client(&m_cs_sender, m_host.c_str(), m_port)) ? true : false;
	return m_connected;
}

void PacketizerDashSender::disconnect()
{
	close_ClientSocket(&m_cs_sender);
	m_connected = false;
}

bool PacketizerDashSender::sendMessage(send_message &msg)
{
	char buf[8192];
