...
#define CAN_CODE_RPM (0x0C)
#define CAN_CODE_SPEED (0x0D)
...
static g_candata g_rpm;
static g_candata g_speed;
static char g_vin[17];
...
MyData::MyData()
{
	...
	m_vin_cts_sem = new sem_t;
	sem_init(m_vin_cts_sem, 0 ,0);
...
static void* can_polling(void* p)
{
	...
	for(;;)
	{
		...
		const int J1979_CAN_ID = md.get_int("J1979_CAN_ID");
		CANSocket* s = md.m_s;
		for(;;)
		{
			{
			...
			usleep(period);
			md.lock();
			size_t VIN_count = md.m_VIN_count;
			md.unlock();
			if(VIN_count == 0)
			{
				unsigned char data[8];
				...
				g_can_write_line = __LINE__;
				sem_timedwait(md.m_vin_cts_sem, &ts);
				memset(data, 0, sizeof(data));
				data[0] = 0x30;
				g_can_write_line = __LINE__;
				res = CAN_write(s, 0x7E0, data, 8);
				if(res.m_err)
				{
					++write_error_count;
					if(write_error_count < 2)
...
static void* serv_client(void* p)
{
	...
	for(;;)
	{
...

		{
			...
			{
				md.lock();
				const size_t count_0C = md.m_0C_count;
				const size_t count_0D = md.m_0D_count;
				md.unlock();
			...
			{
				md.lock();
				send_cmd(cd.m_sockfd, MSG_NOSIGNAL,"getrpm:%s rpm=\"%u\" sec=\"%lu\" usec=\"%lu\" seq=\"%zu\"\r", key.c_str(), g_rpm.data, g_rpm.time.tv_sec, g_rpm.time.tv_usec, md.m_0C_count);
				md.unlock();
			}
			else if("getspeed" == cmd)
			{
				md.lock();
				send_cmd(cd.m_sockfd, MSG_NOSIGNAL,"getspeed:%s speed=\"%u\" sec=\"%lu\" usec=\"%lu\" seq=\"%zu\"\r", key.c_str(), g_speed.data, g_speed.time.tv_sec, g_speed.time.tv_usec, md.m_0D_count);
				md.unlock();
			}
			else if("getvin" == cmd)
			{
				md.lock();
				send_cmd(cd.m_sockfd, MSG_NOSIGNAL, "getvin:%s vin=\"%s\"\r", key.c_str(), g_vin);
				md.unlock();
			}
			else
			{
...
static unsigned int convertToInt(unsigned char* p_data, int p_len)
{
	if(p_len <= 0)
	{
		return 0;
	...
	if(p_len == 1)
	{
		return (unsigned int)p_data[0];
	}

	unsigned int ret_val = p_data[0];

	for(int i=1; i < p_len; i++)
	{
		ret_val <<= 8;
		ret_val |= p_data[i] & 0xFF;
	}

	return ret_val;
}

static void get_processed_can_data(ats::String& p_result, struct can_frame p_msg)
{
	p_result.clear();

	if((p_msg.can_id & 0xFFFFFF00) != 0x700)
	{
		return;
	}
	...
	if(!((p_msg.data[1] == 0x41) && (p_msg.data[0] < 7) && (p_msg.data[0] >=3)))
	{
		return;
	}

	const int length = p_msg.data[0];
	const int val = convertToInt(&p_msg.data[3], length - 2);

	switch(p_msg.data[2])
	{
	case CAN_CODE_RPM:
		ats_sprintf(&p_result, "rpm=%d", val);
		break;

	case CAN_CODE_SPEED:
		ats_sprintf(&p_result, "speed=%d", val);
		break;
...
static void* can_message_handler(void* p)
{
	...
	for(;;)
	{
		...
		if((msg.data[0] == 0x10) && (msg.data[1] == 0x14) && (msg.data[2] == 0x49))
		{
			vin_message_start = 1;
			memset(VIN, 0, sizeof(VIN));
			memcpy(VIN, &(msg.data[5]), 3);
			//SEND CTS right away
			sem_post(md.m_vin_cts_sem);
			if(g_testdata)
			{
				std::stringstream s;
				s << (g_debug_timer->ElapsedTime()) << ":vin MSG["<<sizeof(msg)<<"]:";
				size_t i;
		...
		if((msg.can_dlc == 8) && (msg.data[0] == 0x21) && (vin_message_start == 1))
		{
			memcpy(&VIN[3],&(msg.data[1]), 7);
			vin_message_start = 2;
			if(g_testdata)
			{
				std::stringstream s;
		...
		if((msg.can_dlc == 8) && (msg.data[0] == 0x22) && (vin_message_start == 2))
		{
			memcpy(&VIN[10],&(msg.data[1]), 7);
			...
			{
				char buf[sizeof(g_vin) + 1];

				md.lock();
				memcpy(g_vin, VIN, sizeof(VIN));
				memcpy(buf, g_vin, sizeof(g_vin));
				md.m_VIN_count++;
				md.unlock();
		...
		if(res == 0)
		{
			g_RedStoneData.ObdLastUpdated(updatetime.tv_sec);
		}	
		g_can_message_handler_line = __LINE__;
		const int length = msg.data[0];
		int val = convertToInt(&msg.data[3], length - 2);
		switch(msg.data[2])
		{
		case CAN_CODE_RPM:

			val /= 4;  // added by DRH to compute the actual RPM value.
			g_RedStoneData.RPM(val);
			g_can_message_handler_line = __LINE__;

			send_redstone_ud_msg("message-assembler", 0, "rpm %d\n", val);
			send_redstone_ud_msg("trip-monitor", 0, "TripMonitorSm rpm %d\n", val);
			...
			if(g_testdata)
			{
				...
				size_t i;
				unsigned char *p = (unsigned char *)&msg;
				s << (g_debug_timer->ElapsedTime()) <<":rpm "<<val<<" msg["<<sizeof(msg)<<"]:";
				for(i = 0; i < sizeof(msg); ++i)
				{
					s << std::hex << std::setfill('0') << std::setw(2) << std::uppercase << (p[i] & 0xff) << ',';
				}
		...
		case CAN_CODE_SPEED:
			...
			if(g_testdata)
			{
				...
				size_t i;
				unsigned char *p = (unsigned char *)&msg;
				s << (g_debug_timer->ElapsedTime()) <<":speed "<<val<<" msg["<<sizeof(msg)<<"]:";
				for(i = 0; i < sizeof(msg); ++i)
				{
					s << std::hex << std::setfill('0') << std::setw(2) << std::uppercase << (p[i] & 0xff) << ',';
				}
	...
	for(;;)
	{
		...
		if(g_has_work)
		{
			...
			if(!work_was_set)
			{
				g_has_work_wait_line = __LINE__;
				work_was_set = true;
				ats::system("echo \"set_work key=can-odb2-monitor\"|telnet localhost 41009");
		...
		else
		{
			...
			if((seconds_without_data >= two_minutes) && work_was_set)
			{
				g_has_work_wait_line = __LINE__;
				work_was_set = false;
				ats::system("echo \"unset_work key=can-odb2-monitor\"|telnet localhost 41009");
...
static int start_application(MyData& p_md, int argc, char* argv[])
{
	MyData& md = p_md;
	...
	md.set("J1979_CAN_ID", "0x7DF");
	md.set("polling_period", "500000");
	md.set("command_server_port", "41102");
	md.set("max_command_clients", "16");
	md.set("listen_server_port", "41104");
	...
	for(;;)
	{
		...
		if((msg.can_id & 0xFFFFFF00) == 0x700)
		{
			...
			if(msg.can_dlc > 3)
			{
				...
				else if((msg.data[0] == 0x21) || (msg.data[0] == 0x22))
				{
					g_can_read_line = __LINE__;
					md.post_can_message(msg);
				}
				else if((msg.data[1] == 0x41) && (msg.data[0] < 7) && (msg.data[0] >=3))
				{
...
{
	g_log.open_testdata("can-odb2-monitor");
