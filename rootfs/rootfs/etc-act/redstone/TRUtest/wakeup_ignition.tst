...
#  wakeup_ignition
#
#  test that a unit will wakeup on the Ignition switch
#
#  Procedure:
#	1) set wakeups so that only Ignition is a wakeup
#	2) reboot
#	3) trigger sleep
...
#	2) Unit does not wake up with Ignition
#
#---------------------------------------------------------------------------------------
...
if [ -z "$stage" ]; then
  wait_for_power_monitor
	...
	GetYNResponse $TESTNAME "This test requires an ignition switch attached to the TRULink. Do you have one?" "No ignition switch - test aborted"
  echo "Set the ignition switch to off.  The unit will power down."
  echo "Switch the ignition switch to on after the unit powers down."
  ...
  grep inp1 /tmp/logdir/wakeup.txt> /dev/null
  
  if [ $? != 0 ]; then
    write_error "test failed."
    write_error " unit woke with `cat /tmp/logdir/wakeup.txt`"
  else
    write_log "Unit woke on ignition - test completed succesfully."
  fi
fi
