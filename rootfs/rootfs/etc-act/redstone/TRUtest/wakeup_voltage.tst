...
#  wakeup_voltage
#
#  test that a unit will wakeup on the voltage switch
#
#  Procedure:
#	1) set wakeups so that only voltage is a wakeup
#	2) reboot
#	3) trigger sleep
...
#	2) Unit does not wake up with voltage
#
#---------------------------------------------------------------------------------------
...
if [ -z "$stage" ]; then
  ...
  grep batt_volt /tmp/logdir/wakeup.txt> /dev/null
  
  if [ $? != 0 ]; then
    write_error "test failed."
    write_error " unit woke with `cat /tmp/logdir/wakeup.txt`"
  else
    write_log "Unit woke on voltage - test completed succesfully."
  fi
fi
